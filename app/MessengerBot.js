...
class MessengerBot extends EventEmitter {
  ...
  _startSessionKeepalive() {
    const randMs = (minMin, maxMin) =>
      (Math.floor(Math.random() * (maxMin - minMin + 1)) + minMin) * 60 * 1000;

    const dtsgRefresh = () => {
      if (!this._listening) return;
      if (typeof this.api.refreshFb_dtsg === "function") {
        this.api.refreshFb_dtsg().catch(() => {});
      }
      this._dtsgTimer = setTimeout(dtsgRefresh, randMs(25, 35));
    };

    const presencePing = () => {
      if (!this._listening) return;
      try {
        const ctx = this.api._ctx || this.api.ctx;
        if (ctx && ctx.mqttClient && ctx.mqttClient.connected) {
      ...
      this._presenceTimer = setTimeout(presencePing, randMs(8, 12));
    };
