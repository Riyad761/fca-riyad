...
function login(loginData, options, callback) {
  // E2EE bootstrap shortcut — NEXCA pre-sets this global so that
  // fb-e2ee.cjs can reuse the already-authenticated NEXCA session
  // instead of doing a second Facebook login that would fail.
  if (global._nexcaE2EEAdapter) {
    var _adapter = global._nexcaE2EEAdapter;
    if (utils.getType(options) === 'Function' || utils.getType(options) === 'AsyncFunction') {
      callback = options;
    }
    if (typeof callback === 'function') {
      process.nextTick(function () { callback(null, _adapter); });
      return;
    }
